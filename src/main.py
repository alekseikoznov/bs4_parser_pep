...
def pep(session):
    ...
    if response is None:
        return
    soup = BeautifulSoup(response.text, features='lxml')
    numerical_index_tag = find_tag(soup, 'section', {'id': 'numerical-index'})
    tbody_tag = find_tag(numerical_index_tag, 'tbody')
    tr_tags = tbody_tag.find_all('tr')
    count_status = {}
    results = [('Стадия', 'Количество')]
    total = 0
    for tr_tag in tqdm(tr_tags):
        ...
        current_pep_url = urljoin(PEP_URL, current_pep_link)
        response = get_response(session, current_pep_url)
        if response is None:
            continue
        soup = BeautifulSoup(response.text, features='lxml')
        current_status = soup.find(
            text='Status').parent.find_next_sibling().text
        if current_status not in preview_status:
            logging.info(f'Несовпадающие статусы:\n'
                         f'{current_pep_url}\n'
                         f'Статус в карточке: {current_status}\n'
                         f'Ожидаемые статусы: {preview_status}\n')
        if current_status not in count_status:
            count_status[current_status] = 0
        count_status[current_status] += 1
        total += 1
    for item in count_status.items():
        results.append(item)
